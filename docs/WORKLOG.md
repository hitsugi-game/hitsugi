...
- **Ship Check・履歴整理**: 独立機能監査とsecurity監査は最終SHIP-with-notes/blocking 0。briefのローカル絶対パスは未push祖先commitへfixup/autosquashし、旧commitをmainから到達不能にした。指定dirty 3件は一時stashし、復元後の各diff hash一致を確認。post-rewriteでも全Vitest 53/783、build/lint/data/closure/manifestが合格し、`origin/main..HEAD`追加差分/履歴の絶対パス・email・秘密候補0、staging 0を確認した。既公開baselineの`assets_src/codex_image_batch.md`に残る絶対パス、特殊影接触時のcheckpoint未保存、Actions major tag固定は今回差分外のnon-blocking noteとして残す。
- **公開**: M47B `10c5b35`、M47C `c6e06f6`をmainへpush。GitHub Actions run `30188332927`はbuild/test/deploy全job成功。`https://hitsugi-game.github.io/hitsugi/?v=c6e06f6`をブラウザで開かずHTTP取得し、HTML 200、JS `index-B_r6ojIm.js` 200、CSS `index-DWefqh9n.css` 200、bundle内の「行動候補」「確定ではない」「敵影が速まり」を確認した。

## 2026-07-26（M51 出立seedによる夜藪変奏）

- **依頼/判断**: 毎回同じダンジョンを、宝箱・報酬・内容が変わる遠征へ改める。単純な毎描画乱数はリロード引き直しとcheckpoint不一致を生むため採用せず、「新しい出立では変わる／同じ遠征の再開では変わらない」一遠征一seedを契約とした。
- **実装**: `DungeonRun.runSeed`を出立時に生成してsaveへ保持。一般階層は接続を保存する反転と、入口から到達可能な床へのPOI再配置を合成する。宝箱数だけ1〜3へ変動し、祠・焚火・石碑は既存個数を保つ。floor seedも出立ごとに変えるため敵影と地表propの配置が変わる。作家演出済みstage contractは背景との位置ずれを避け、地形を固定する。
- **報酬/事件**: 宝箱、祠事件、事件の選択結果、焚火加護を`runSeed/floor/座標/channel`の局所RNGへ接続。同じcheckpointから再開しても内容が一致する一方、別出立では変わる。報酬率、地域tier、目利き、事件効果は従来値のまま。
- **互換/検証**: runSeedはoptionalとし、旧checkpointは従来床へfallback。不正seedはsave guardで拒否する。通常171層＋常夜百層の全271層の到達性、必須tile、POI個数、同seed一致、別seed差、専用stage固定、宝箱再開一致、checkpoint保存をfocused testへ追加。初回全回帰で地域地相文が最新ログである既存契約との順序衝突1件を検出し、変奏説明を地相文の直前へ移して修復した。focused 37、全Vitest 54 files/788、型、lint、data 0 errors/既存rank warn 1、closure 69、manifest 9、production build、専用stageのPC1280/mobile390 Playwright 2/2に合格。main chunk約1.49MBの既存警告は継続。push/deployは未実施。
- **Forge Round 1**: 固定5軸で独立評価し、A/B/C/D/E=`4/4/3/5/4`、blocking 1でFAIL。新規seed遠征は合格圏だったが、seed欠損を許す旧checkpointだけ`dungeonTileRng`がglobal RNGへfallbackし、`continueGame`の時刻seedにより宝箱・祠・事件結果・焚火候補を引き直せると判明した。
- **欠陥限定修正**: 旧checkpointは地形を従来固定床のまま維持し、地域IDと固定party ID列から安定内容seedを導出する。異なるglobal RNGで同じ旧checkpointを二度`continueGame`し、宝箱、祠、成功率付き事件、焚火加護を実store経路で比較する回帰を追加。focused 4 files/42 tests、型、lintに合格。Round 2独立閉鎖と全gateは後段に記録する。
...
## 2026-08-05（GitHub Actions Node.js 24ランタイム移行・公開済み）
...
- **公開検証**: `42b978f`をmainへpushし、GitHub Actions run `31011617744`のverify/deployが成功。公開HTMLと55 JS/CSS/deferred resourcesは全てHTTP 2xx、commit markerは`42b978f`と一致した。verify/deploy両jobのannotationは0件で、Node.js 20非推奨警告が解消したことをAPIでも確認した。
